...
type SolicitudCardProps = {
	caso: HelpRequestData & { special_situations: any };
	showLink?: boolean;
	showEdit?: boolean;
	more?: boolean;
};

export default function SolicitudCard({ caso, showLink = true, showEdit = false, more }: SolicitudCardProps) {
				...
				<div className="flex flex-col sm:flex-row items-start gap-4 sm:items-center justify-between border-b border-gray-900/10 px-6 py-4">
					<div className="flex flex-row justify-between">
						<div className="flex flex-row gap-4 items-center">
							<AlertTriangle
								className={`h-10 w-10 flex-shrink-0 ${updateUrgency === 'alta'
									? 'text-red-500'
									: updateUrgency === 'media'
										? 'text-yellow-600'
										: 'text-green-600'
							...
							<div className="flex flex-col">
								<span
									className={`text-lg font-bold ${updateUrgency === 'alta'
										? 'text-red-500'
										: updateUrgency === 'media'
											? 'text-yellow-600'
											: 'text-green-600'
					...
					<div className="flex flex-row justify-center items-center gap-2">
						...
						<div
							className={`flex items-center justify-center rounded-full px-4 py-2 ${caso.status === 'finished'
				...
				<div className="flex flex-col items-start justify-between border-t border-gray-900/10 sm:flex-row sm:items-center px-6 py-4">
					...
					<div className="flex flex-col pt-4 sm:pt-0 sm:flex-row w-full sm:w-auto justify-end gap-2">
						...
						{showLink && (
							...
							>
								{more ? 'Ver solicitud completa' : 'Ver solicitud'}
							</Link>
						)}
