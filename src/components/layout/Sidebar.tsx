...
export default function Sidebar({ isOpen, toggleAction }: SidebarProps) {
	...
	useEffect(() => {
		async function fetchRequests() {
			const url = process.env.NEXT_PUBLIC_BASE_URL + '/api/solicitudes/usuario';
			try {
		...
		async function fetchOffers() {
			const url = process.env.NEXT_PUBLIC_BASE_URL + '/api/ofertas/usuario';
			try {
				const response = await fetch(url);
				if (!response.ok) {
					console.log(`Error fetching solicitudes: ${response.status}`);
								...
								item.isHref ? (
									<button
										...
										className={`w-full text-left transition-colors
											...
											${item.isHome ? 'p-3 rounded-lg flex items-center gap-2 hover:bg-gray-50' : `p-4 rounded-lg ${pathname === item.path ? 'bg-gray-100 shadow-sm' : 'hover:bg-gray-50'} ${item.highlight ? 'bg-red-50 border-2 border-red-200 hover:bg-red-100 animate-pulse' : ''
												}`
											}`}
									>
										<>
									...
									<button
										...
										className={`w-full text-left transition-colors ${item.isHome
											? 'p-3 rounded-lg flex items-center gap-2 hover:bg-gray-50'
											: `p-4 rounded-lg ${pathname === item.path ? 'bg-gray-100 shadow-sm' : 'hover:bg-gray-50'} ${item.highlight ? 'bg-red-50 border-2 border-red-200 hover:bg-red-100 animate-pulse' : ''
											}`
											}`}
									>
										<>
